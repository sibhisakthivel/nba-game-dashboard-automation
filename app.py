...
from config import TEAM_NAMES, STATS, DEF_BUCKET_TOP, DEF_BUCKET_MIDDLE
...
def get_games_without_teammates(pbs, player_id, teammate_ids):
    """
    Get game IDs where specified teammates were out.
    ...
    player_id : int
        Player personId
    teammate_ids : list
        List of teammate identifiers (personId int or familyName str)
    
    Returns:
    --------
    set
        Set of game_ids where all specified teammates were out
    """
    if not teammate_ids:
        player_pbs = pbs[pbs["personId"] == player_id]
        return set(player_pbs["game_id"])
    
    player_pbs = pbs[pbs["personId"] == player_id].copy()
    player_game_ids = set(player_pbs["game_id"])

    games_without_teammates = set(player_game_ids)
    for teammate_id in teammate_ids:
        teammate_games = get_teammate_game_ids(pbs, teammate_id, player_game_ids)
        games_without_teammates = games_without_teammates - teammate_games

    return games_without_teammates

def render_teammate_filtered_plot_table(
    game_ids,
    plot_title,
    player_id,
    prop_line,
    ...
    prop_line : float
        Prop line value
    pbs : pd.DataFrame
        Full player box scores dataframe
    tbs : pd.DataFrame
        Team box scores dataframe
    daily_ranks : pd.DataFrame
        Daily defensive ranks dataframe
    matchup_home_away : str or None
        Home/away filter for matchup
    matchup_opp_def_bucket : str or None
        Opponent defensive bucket filter for matchup
    """
    if len(game_ids) == 0:
        return
    
    # Hit rate table and plot side by side
    col_table, col_plot = st.columns([0.35, 0.65])
    
    with col_table:
        # Add spacer to align table top with plot title
        st.markdown("<div style='height: 2.5rem;'></div>", unsafe_allow_html=True)
        
        # Filter pbs for hit rate calculation
        filtered_pbs = pbs[
            (pbs["personId"] == player_id) &
            (pbs["game_id"].isin(game_ids))
        ].copy()
        ...
        summary_table_filtered = summary_table[
            ...
            (summary_table["Category"].notna()) &
            (summary_table["Category"] != "")
        ].copy()
        
        if len(summary_table_filtered) > 0:
    ...
    with col_plot:
        # Add spacer to align plot with table
        st.markdown("<div style='height: 2.5rem;'></div>", unsafe_allow_html=True)
        
        # Plot (use full pbs for rolling averages, filter games shown)
        filtered_fig, ax = plot_player_scoring(
            player_id,
            prop_line,
            pbs,
            tbs,
            daily_ranks,
            teammate_ids=None,  # No markers needed in filtered plots
            game_ids_filter=game_ids,
            show_legend=False,  # Hide legend for filtered plots
            stat=stat
        )
        # Center title over plot
        filtered_fig.suptitle(plot_title, fontsize=24, fontweight='bold', y=0.98)
        # Use smaller size to fit next to table
        filtered_fig.set_size_inches(20, 10)
        filtered_fig.set_dpi(100)
        filtered_fig.tight_layout(rect=[0, 0, 1, 0.96])  # Leave space for suptitle
...
if "selected_teammates" not in st.session_state:
    st.session_state.selected_teammates = []  # Max 2 teammates
...
if st.session_state.selected_team is not None:
    ...
    if len(players_df) > 0:
        ...
        if st.session_state.selected_player_id is not None:
            ...
            # Dropdown 4: Stat + Prop Line Input (only visible after opponent is selected)
            if st.session_state.selected_opponent is not None:
                st.markdown("### Select Stat")
                stat_options = list(STATS.keys())
                selected_stat = st.selectbox(
                    "Stat",
                    options=stat_options,
                    format_func=lambda k: STATS[k]["label"],
                    index=stat_options.index(st.session_state.selected_stat),
                    key="stat_selectbox"
                )
                ...
                prop_line = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=300.0,
                    value=STATS[selected_stat]["default_line"],
                    step=0.5,
                    key=f"prop_line_input_{selected_stat}"
                )

                st.session_state.prop_line = prop_line
                
                # Teammate Selection (max 2)
                if st.session_state.selected_player_id is not None:
                    st.markdown("### Select Injured Teammates (Max 2)")
                    # Reuse already-loaded players_df, exclude the selected player
                    teammates_df = players_df.copy()
                    teammates_df = teammates_df[teammates_df["person_id"] != st.session_state.selected_player_id]
                    
                    if len(teammates_df) > 0:
                        teammate_options = [
                            (row["person_id"], f"{row['display_name']} ({row['avg_ppg']:.1f} PPG)")
                            for _, row in teammates_df.iterrows()
                        ]
                        
                        # Get current selections
                        current_selections = []
                        if st.session_state.selected_teammates:
                            for tm_id in st.session_state.selected_teammates:
                                for pid, display in teammate_options:
                                    if pid == tm_id:
                                        current_selections.append((pid, display))
                                        break
                        ...
                        selected_teammates = st.multiselect(
                            ...
                            max_selections=2,
                            key="teammate_multiselect"
                        )
                        
                        # Update session state (extract just the IDs)
                        if selected_teammates:
                            st.session_state.selected_teammates = [tm[0] for tm in selected_teammates]
                        else:
                            st.session_state.selected_teammates = []
                    else:
                        st.session_state.selected_teammates = []
                ...
                if (st.session_state.selected_team is not None and
                    ...
                    with st.spinner("Loading player data..."):
                        pbs_player = load_process_pbs_from_db(engine, person_id=st.session_state.selected_player_id)
                        if len(pbs_player) > 0:
                            player_team_abbrev = pbs_player["teamTricode"].iloc[0]
                            pbs = load_process_pbs_from_db(engine, person_id=None, team_abbrev=player_team_abbrev)
                        else:
                            pbs = pbs_player
                    ...
                    <div style='text-align: center; padding: 1rem; background-color: rgba(0,0,0,0.1); border-radius: 10px; margin: 1rem 0;'>
                        <div style='display: flex; justify-content: center; align-items: center; gap: 1rem; margin-bottom: 1rem;'>
                            <h2 style='margin: 0; text-align: right;'>{away_name}</h2>
                    ...
                    if len(player_name_rows) > 0:
                        player_name_row = player_name_rows.iloc[0]
                        player_display_name = f"{player_name_row['firstName']} {player_name_row['familyName']}"
                    else:
                        player_display_name = f"Player {st.session_state.selected_player_id}"
                    
                    # Render player scoring plot and hit rate table
                    stat_display_label = STATS[st.session_state.selected_stat]["label"]
                    st.markdown(f"<h2 style='text-align: center;'>{player_display_name} - Over {st.session_state.prop_line} {stat_display_label}<br></h2>", unsafe_allow_html=True)
                    
                    # Render player scoring plot (full width)
                    # Pass up to 2 teammates for markers on main plot
                    main_plot_teammates = st.session_state.selected_teammates[:2] if st.session_state.selected_teammates else None
                    player_scoring_fig, _ = plot_player_scoring(
                        ...
                        tbs,
                        daily_ranks,
                        teammate_ids=main_plot_teammates,
                    ...
                    with col_table:
                        ...
                        if opp_def_rank is not None:
                            if opp_def_rank <= DEF_BUCKET_TOP:
                                matchup_opp_def_bucket = "Top 10 Defense"
                            elif opp_def_rank <= DEF_BUCKET_MIDDLE:
                    ...
                    # =========================================================
                    # Filtered Teammate Plots Section
                    # =========================================================
                    if st.session_state.selected_teammates:
                        st.markdown("---")
                        st.markdown("<h2 style='text-align: center; margin-top: 2rem; margin-bottom: 1rem;'>Performance Without Selected Teammates</h2>", unsafe_allow_html=True)
                        
                        # Get teammate names for display
                        pbs_names = pbs.copy()
                        pbs_names["firstName"] = pbs_names["firstName"].str.lower()
                        pbs_names["familyName"] = pbs_names["familyName"].str.lower()
                        
                        teammate_names = []
                        for teammate_id in st.session_state.selected_teammates:
                            if isinstance(teammate_id, str):
                                ...
                                if len(name_rows) > 0:
                                    ...
                                    teammate_names.append(f"{name_row['firstName'].title()} {name_row['familyName'].title()}")
                                else:
                                    teammate_names.append(teammate_id.title())
                            else:
                                name_rows = (
                                ...
                                if len(name_rows) > 0:
                                    ...
                                    teammate_names.append(f"Teammate {len(teammate_names) + 1}")
                        
                        # Plot 1: Games without teammate 1
                        if len(st.session_state.selected_teammates) >= 1:
                            game_ids_1 = get_games_without_teammates(
                                pbs, 
                                st.session_state.selected_player_id, 
                                [st.session_state.selected_teammates[0]]
                            )
                            
                            render_teammate_filtered_plot_table(
                                game_ids_1,
                                f"Games Without {teammate_names[0]}",
                                st.session_state.selected_player_id,
                                st.session_state.prop_line,
                                pbs,
                                tbs,
                        ...
                        if len(st.session_state.selected_teammates) >= 2:
                            game_ids_2 = get_games_without_teammates(
                                pbs, 
                                st.session_state.selected_player_id, 
                                [st.session_state.selected_teammates[1]]
                            )
                            
                            render_teammate_filtered_plot_table(
                                game_ids_2,
                                f"Games Without {teammate_names[1]}",
                                st.session_state.selected_player_id,
                                st.session_state.prop_line,
                                pbs,
                                tbs,
                        ...
                        if len(st.session_state.selected_teammates) >= 2:
                            game_ids_both = get_games_without_teammates(
                                pbs, 
                                st.session_state.selected_player_id, 
                                st.session_state.selected_teammates
                            )
                            ...
                            render_teammate_filtered_plot_table(
                                game_ids_both,
                                f"Games Without {teammate_names[0]} AND {teammate_names[1]}",
                                st.session_state.selected_player_id,
                                st.session_state.prop_line,
                                pbs,
                                tbs,
