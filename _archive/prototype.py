...
def get_games_without_teammates(pbs, player_id, teammate_ids):
    ...
    # Find games where each teammate was out
    ...
    for teammate_id in teammate_ids:
        ...
        # Keep only games where this teammate was out
        ...
        if st.session_state.selected_player_id is not None:
            ...
            if st.session_state.selected_opponent is not None:
                ...
                if st.session_state.selected_player_id is not None:
                    ...
                    # Get all teammates (exclude the selected player)
                    teammates_df = get_players_by_team(engine, st.session_state.selected_team)
                ...
                if (st.session_state.selected_team is not None and
                    ...
                    # Load player data - need to load ALL team players for teammate analysis
                    # First load the selected player's data
                    pbs_player = load_process_pbs_from_db(engine, person_id=st.session_state.selected_player_id)
                    
                    # If teammates are selected, we need to load their data too
                    # For now, load all players from the same team to support teammate analysis
                    # Get team from the player's data
                    if len(pbs_player) > 0:
                        player_team_abbrev = pbs_player["teamTricode"].iloc[0]
                        # Load all players from the same team for teammate lookup
                        # We'll filter to just the team's games to keep it efficient
                        pbs_team = load_process_pbs_from_db(engine, person_id=None)  # Load all players
                        # Filter to just players from the same team
                        pbs = pbs_team[pbs_team["teamTricode"] == player_team_abbrev].copy()
                    else:
                        pbs = pbs_player
